...
for path, dirs, files in os.walk(os.getcwd()):
    for file in files :
        if '.object' in file:
            ...
            t_json = json.loads(removecomments(t_string), strict=False)
            if 'novakidDescription' in t_json:
                t_desc = t_json['novakidDescription']
                p_list.append(dict(path = os.path.relpath(path,common_prefix),file = file, name = t_json['novakidDescription'], desc = t_desc))
